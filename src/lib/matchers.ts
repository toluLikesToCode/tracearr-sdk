...
import { TracearrSDKDefaultError } from "../models/errors/tracearr-sdk-default-error.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | TracearrSDKDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | TracearrSDKDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new TracearrSDKDefaultError(
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new TracearrSDKDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
